$(function(){
	...
	//
	// 	size: new AMap.Size(40, 50), // 图标真实尺寸
	// 	image: 'images/icon_truck.png',  // Icon的图像
	// 	imageSize: new AMap.Size(20, 25)  // 根据所设置的大小调整图标缩进尺寸
	// });
	//
    // var maker;
	...
	var point = [
		[125.205366, 43.841323],
		[125.218494,43.84985],
		[125.19079,43.837887],
		[117.1795,39.1082],
		[125.207554,43.84196],
		[116.289509,40.017091],
		[117.301462,31.88607],
		[113.196485,23.153606],
		[102.878406,24.949265],
		[121.673291,31.305591],
		[121.581643,30.914357],
		[97.323234,33.100069]
	];

	var myMap11 = new AMap.Map('myMap11', {
		resizeEnable: true,
		zoom: 4,
		mapStyle: 'amap://styles/fresh',
		center: [103.752171, 36.068716],
	...
	// 创建聚合实例并添加到地图
	var markerCluster = new AMap.MarkerClusterer(myMap, markers, {
		gridSize: 80,  // 聚合计算时网格的像素大小，默认60
		minClusterSize: 2,  // 聚合的最小数量
		maxZoom: 15,  // 最大的聚合级别，大于该级别就不进行相应的聚合
		averageCenter: true,  // 聚合的方式，平均中心还是聚合中心最靠近中心的点
	});
	...
	$('.summaryBtn').on('click',function(){
		...
		$('.popup').show();
		$('.popup').width('3px');
		$('.popup').animate({height: '100%'},400,function(){
			$('.popup').animate({width: '100%'},400);
		});
		setTimeout(summaryShow,800);
	});

	// $('.summaryBtn').on('click', function() {
    // $('.sidebar').width(0); // 初始化宽度为0
    // $('.sidebar').show(); // 显示侧边栏
    // $('.sidebar').animate({width: '250px'}, 400); // 动画使侧边栏宽度变为250px
	// });
